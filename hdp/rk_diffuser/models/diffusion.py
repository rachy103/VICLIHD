...
class GaussianDynDiffusion(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # --- 수정 ---
        # (필요시 별칭: 'mse'/'weighted'가 들어와도 'l2'로 매핑)
        aliases = {"mse": "l2", "weighted": "l2"}
        loss_key = aliases.get(loss_fn, loss_fn)

        if loss_key == "state_chamfer":
            self._loss_fn = Losses[loss_key](loss_weights, coverage_weight, loss_type)
        else:
            LossClass = Losses[loss_key]
            try:
                # 일부 구현은 action_dim을 요구함
                self._loss_fn = LossClass(loss_weights, self._action_dim)
            except TypeError:
                # 어떤 구현은 action_dim이 없음
                self._loss_fn = LossClass(loss_weights)
