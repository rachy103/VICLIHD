...
from .. import utils
from .rl_bench_env import CustomRLBenchEnv
...
class RLBenchDataset(Dataset):
    ...
    def _get_env(self, task, headless=True, use_traj=False):
        ...
        env = CustomRLBenchEnv(
            ...
            episode_length=200,
            dataset_root=os.path.join(self._data_raw_path, 'train' if self._training else 'eval'),
            headless=headless,
            time_in_state=True,
        )
    ...
    def _get_demos(self, task, ret_dict):
        def _get_demos_fn():
            os.environ['COPPELIASIM_ROOT'] = '/home/yun/.local/bin/CoppeliaSim'
