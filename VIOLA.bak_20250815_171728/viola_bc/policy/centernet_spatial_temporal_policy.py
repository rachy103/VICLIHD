...
class CenterNetSpatialTemporalPolicy(BasePolicy):
    def __init__(self,
        ...
        # OmegaConf struct 모드 호환: dict로 복사 후 보강
        ...
        _projection_kwargs = dict(policy_cfg.projection.network_kwargs)
        _projection_kwargs["input_shape"] = input_shape
        print(input_shape)
        self.projection = eval(policy_cfg.projection.network)(**_projection_kwargs)
        ...
        ###
        #####################################################
        _bbox_norm_kwargs = dict(policy_cfg.bbox_norm.network_kwargs)
        _bbox_norm_kwargs["input_shape"] = input_shape
        self.bbox_norm = eval(policy_cfg.bbox_norm.network)(**_bbox_norm_kwargs)
    ...
    def encode_fn(self, batch):
        print("[PROOF] ==> VIOLA Policy (encode_fn) is being used.")
        # 증거 로그: VIOLA encode_fn 호출
        print("[PROOF] VIOLA.encode_fn called")
        #####################################################
