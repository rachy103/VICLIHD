...
try:
    # robomimic >= 0.5.0에서 Randomizer는 obs_core로 이동
    from robomimic.models.obs_core import Randomizer
except Exception:
    # 구버전 호환 (예외적으로 base_nets 경로 시도)
    from robomimic.models.base_nets import Randomizer
# robomimic >= 0.5.0에서는 CropRandomizer가 obs_core로 이동
try:
    from robomimic.models.obs_core import CropRandomizer
except Exception:
    from robomimic.utils.image_utils import CropRandomizer
...
class CLIPVisionEmbedding(nn.Module):
    """
    실제 CLIP 비전 백본을 사용해 패치 토큰을 2D 피처맵으로 복원한 뒤,
    1x1 conv로 채널을 맞춰 반환합니다. (ViT-L/14 기준 16x16 그리드)
    """
    def __init__(self,
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x: [B, C, H, W] (float, 0~1 권장)
        assert x.ndim == 4, f"Expected 4D tensor [B,C,H,W], got {x.shape}"
        B = x.shape[0]
        dev = self._ensure_device(x)
        self._vision = self._vision.to(dev)

        # transformers 프로세서는 list of PIL/tensors 형태를 기대
        # 텐서를 각 샘플로 분리해 전달
        imgs = [xi.detach().cpu() for xi in x]
        proc = self._processor(images=imgs, return_tensors="pt")
        pixel_values = proc["pixel_values"].to(dev)
        out = self._vision(pixel_values=pixel_values)
        # last_hidden_state: [B, 1 + N, D] (CLS + 패치)
        tokens = out.last_hidden_state[:, 1:, :]
        B, N, D = tokens.shape
        S = int((N) ** 0.5)
        # grid 크기와 일치 확인, 불일치 시 강제 reshape 가능
        if S * S != N:
            raise RuntimeError(f"CLIP 토큰 수가 정사각형이 아닙니다: N={N}")
        fmap = tokens.transpose(1, 2).reshape(B, D, S, S)
        fmap = self._proj(fmap)
        return fmap

    def output_shape(self, input_shape):
        # 입력과 무관하게 CLIP 프로세서가 224로 리사이즈 → grid=16
        return (self.out_channels, self._grid, self._grid)

class CLIPImgEmbeddingFallback(nn.Module):
    """
    간단한 대체 CLIP 이미지 인코더.
    외부 모델 다운로드 없이 ResnetConv를 백본으로 사용해 동일한 인터페이스를 제공합니다.
    """
    def __init__(self,
                 remove_layer_num=4,
                 img_c=3,
    ...
    def output_shape(self, input_shape):
        return self._backbone.output_shape(input_shape)
...
class DataAugGroup(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_random_erasing:
            re_args = dict(aug_kwargs.get("random_erasing", {}))
            # torchvision>=0.14는 scale, ratio가 시퀀스여야 함. 단일 스칼라면 보정
            def _ensure_seq(val, default):
                try:
                ...
                if isinstance(val, Sequence) and not isinstance(val, (str, bytes)):
                    return val
                return default
            # Debug print to inspect args at runtime
            print("[DataAugGroup] RandomErasing args:", re_args, type(re_args.get("scale")))
            if "scale" in re_args:
                val = _ensure_seq(re_args["scale"], (0.02, 0.33))
                re_args["scale"] = tuple(val)
            if "ratio" in re_args:
                val = _ensure_seq(re_args["ratio"], (0.3, 3.3))
                re_args["ratio"] = tuple(val)
            random_erasing = torchvision.transforms.RandomErasing(**re_args)
